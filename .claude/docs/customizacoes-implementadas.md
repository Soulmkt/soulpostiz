...
| 13/09/2026 | **Tela Comentários (SPTZ-6)**: item "Comentários" no menu principal (depois de Plugs), rota `/soul/comments` com abas por status e contagem (Fila, Prontas, Automáticas, Respondidas no app, Ignoradas, Novas), cartão por comentário (autor, tempo, minutos após o post, selo "dentro da 1ª hora", rótulo em português, link do post, legenda), ações Responder agora / Gerar sugestão / Deixar o automático responder / Descartar, editor de regras por cliente (modo, voz, fontes, e-mails), botão Rodar agora. Lista da API enriquecida com `media {permalink, caption}`. Provado em produção 13/09 03:30 UTC com os seeds (Fila 5, Prontas 2 com os textos prontos no campo, Ignoradas 1). No deploy o backend do pm2 travou em silêncio no boot (502 por 10 min); `pm2 restart backend` resolveu e o script de deploy passou a conferir a API e reiniciar sozinho. | `apps/frontend/src/components/soul/comments/soul.comments.tsx`, `apps/frontend/src/app/(app)/(site)/soul/comments/page.tsx`, `apps/frontend/src/components/layout/top.menu.tsx` (core, PATCHES.md), `apps/backend/src/soul/soul.comments.controller.ts` | `soulpostiz:v2.23.0-soul.6` (commit c833c908) | SPTZ-6 |
...
Produção em 13/09/2026 03:20 UTC: `postiz.soulmkt.com.br` roda
`soulpostiz:v2.23.0-soul.6` (soul.1 subiu em 12/09 22:20 UTC). Imagem oficial anterior
