...
| `libraries/nestjs-libraries/src/database/prisma/database.module.ts` | registrar `SoulCommentsRepository`, `SoulCommentsCollectorService` e `SoulCommentsClassifierService` (3 imports + 3 providers, marcados `// soulpostiz`) | reinserir os imports e as duas linhas em `providers` (o `exports` devolve `providers`) | SPTZ-3 |
